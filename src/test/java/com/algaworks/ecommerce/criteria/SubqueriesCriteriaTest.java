...
//        Todos os produtos que já foram vendidos por um preco direrente do atual
//        String jpql = "select p from Produto p where p.preco <> " +
//                "ANY (select precoProduto from ItemPedido where produto = p)";
...
//                "p.id in (select p2.id from ItemPedido i2 " +
//                "join i2.pedido join p2.produto pro2 where pro2.preco > 100";
...
//       String jpql = "select p from Pedido p where " +
//               "p.total > (select avg(preco) from Pedido)";
